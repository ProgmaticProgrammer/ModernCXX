...
QString Model::op() const
{
    return op_;
}

void Model::setOp(const QString &op)
{
    op_ = op;
}

double Model::rhs() const
{
    return rhs_;
}

void Model::setRhs(double rhs)
{
    rhs_ = rhs;
}

double Model::lhs() const
{
    return lhs_;
}

void Model::setLhs(double lhs)
{
    lhs_ = lhs;
}
...
//GuiInterface::GuiInterface(QWidget *parent, Model *model) : QWidget(parent), model_(model)
//{
...
//    connect(this->model_,  &Model::resultChanged, recentInput, &QLineEdit::setText);
//}
...
//    if (!r.contains('.')) {
//        model_->setResult(r + '.');
//        if (model_->state() == Model::State::WAITING_LHS) model_->setState(Model::State::LHS_INPUTTING);
//        else if (model_->state() == Model::State::WAITING_RHS) model_->setState(Model::State::RHS_INPUTTING);
//    }
//}
...
//    double result = HUGE_VAL;
//    QString o = model_->op();
//    model_->setRhs(operand);

//    if (o != "" && checkOpPrecondition(o, operand)) {
//        if (o == tr("+")) {
//            result = model_->lhs() + model_->rhs() ;
//        } else if (o == tr("-")) {
//            result = model_->lhs() - model_->rhs() ;
...
//                expression->setText( expression->text() + model_->result() + clickedOperator);
//                break;
//            case Model::State::RHS_INPUTTING:
...
//        doBinaryCalculation(model_->rhs());
//    }
...
//    else {
//        model_->setResult("0");
//        if (model_->state()==Model::State::LHS_INPUTTING) model_->setState(Model::State::WAITING_LHS);
//        else if (model_->state()==Model::State::RHS_INPUTTING) model_->setState(Model::State::WAITING_RHS);
//    }
//}
