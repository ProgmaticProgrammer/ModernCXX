...
class ConcretePublisher : private Publisher
{
public:
    ConcretePublisher();
    ~ConcretePublisher();
...
private:
    void notifyImpl(std::shared_ptr<EventData>);
...
void ConcreteObserver::notifyImpl(std::shared_ptr<EventData> data)
{
    notified_ = true;
...
void PublisherObserverTest::testRaiseEvent()
{
    ...
    publisher_->notify("multipleEventTwo", std::make_shared<EventData>());

    QCOMPARE(p1->notified(), false);
    QCOMPARE(p1->notified(), false);

    // check that raising observed event was indeed observed
    publisher_->notify("multipleEventOne", std::make_shared<EventData>());
